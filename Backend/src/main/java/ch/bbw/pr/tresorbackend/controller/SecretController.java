...
public class SecretController {
   ...
   public ResponseEntity<String> createSecret2(
      ...
      if (isForbiddenForUserEmail(newSecret.getEmail())) {
         JsonObject obj = new JsonObject();
         obj.addProperty("message", "You are not allowed to create secrets for this user.");
   ...
   public ResponseEntity<List<Secret>> getSecretsByUserId(
      ...
      if (user == null) return ResponseEntity.notFound().build();
      if (isForbiddenForUserEmail(user.getEmail())) {
      ...
      if (secrets.isEmpty()) return ResponseEntity.ok(List.of());
   ...
   public ResponseEntity<List<Secret>> getSecretsByEmail(
      ...
      if (isForbiddenForUserEmail(credentials.getEmail())) {
         return ResponseEntity.status(HttpStatus.FORBIDDEN).body(List.of());
      }
   ...
   public ResponseEntity<String> updateSecret(
      ...
      if (isForbiddenForUserEmail(newSecret.getEmail())) {
         JsonObject obj = new JsonObject();
         obj.addProperty("answer", "You are not allowed to update secrets for this user");
         return ResponseEntity.status(HttpStatus.FORBIDDEN).body(new Gson().toJson(obj));
      }
   ...
   public ResponseEntity<String> deleteSecret(@PathVariable Long id) {
      ...
      User user = userService.getUserById(dbSecret.getUserId());
      if (user == null) {
         return ResponseEntity.status(HttpStatus.NOT_FOUND).body("User not found");
      }
      if (isForbiddenForUserEmail(user.getEmail())) {
         return ResponseEntity.status(HttpStatus.FORBIDDEN).body("You are not allowed to delete this secret");
      }
   ...
   private boolean isForbiddenForUserEmail(String email) {
      ...
      return !(isAdmin || authentication.getName().equalsIgnoreCase(email));
   }
}
